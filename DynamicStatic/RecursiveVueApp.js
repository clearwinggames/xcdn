Vue.component('recursive-vue-app',
{
  ...
  methods: {
	  ...
	  performRouterAdd: function(path, route, routeToAddTo) 
	  {
				console.log('Adding child relationship: ' + path + '; vs ' + routeToAddTo.path + ' (' +  
				 ...
				 else if (path.indexOf('/') > 0)
				 {
					 ...
					 this.assignAsChild(route, this.router.getRoutes());
					 return;
				 }
				 else {
	  ...
	  assignAsChild: function(route, routes) {
	  ...
	  isParent: function(routeChild, routeOther) {
			if (this.isAncestor(routeChild, routeOther) || this.isOldestAncestor(routeChild, routeOther)) {
				...
				if (routeChild.path.indexOf(otherPath) >= 0 && (routeChild.path.endsWith(otherPath) || routeChild.path.substring(1).endsWith(otherPath) || routeChild.path.substring(1).endsWith(otherPath.substring(1)) || routeChild.path.endsWith(otherPath.substring(1)))) {  
	  ...
	  singleSlash: function(urlStart, entryUrlEnd)
	  {
		  ...
		  if (typeof fullUrl == 'undefined') return urlStart + '/' + entryUrlEnd;
	  ...
	  preprocessActiveEntryTemplate: function() {
		  ...
		  var entry = this.getActiveEntry(); // figure out how to get this
		  if (entry.template.length == 0) return 'todo: implement this';
  ...
  <div>
  	<div v-if="structure != null && structure.entries != null">
  	 <div v-for="entry in structure.entries">
	 	<div v-show="true">
			Component: ( {{ entry.title }} ) Is Active Component? {{ isEntryInUrl(entry) }}
			<div v-show="isEntryInUrl(entry)">
